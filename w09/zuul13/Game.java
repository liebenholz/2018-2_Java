...
public class Game {
    ...
    private void goRoom(Command command) {
        ...
        if (player.moveTo(direction) == -1) System.out.println("그 쪽에는 문이 없습니다!");
    ...
    private void take(Command cmd) {
    	if(!cmd.hasSecondWord()) {
    		System.out.println("take 명령어는 인자가 필요합니다.");
    		System.out.println("집어 들 아이템 명을 입력하시오.");
    		System.out.println(); return;
    	}
    	String item = cmd.getSecondWord();
    	player.takeItem(item);
    	
    	if(item==null) System.out.println("아이템 집을 수가 없어요");
    ...
    private void drop(Command cmd) {
    	...
    	if(item==null) System.out.println("아이템을 버릴 수가 없어요");
    	else printItems(player.getItems());
     ...
     * 아이템들의 총 무게와 이 선수가 들 수 있는
     * @param items
     */
    private void printItems(List<Item> items) {
    	int sum = 0;
    	System.out.println("<현재 가진 아이템>");
    	...
    	System.out.print("<현재 가지고 있는 아이템들의 총 무게 : " + sum);
    	System.out.println(", 들 수 있는 최대 무게 : " + player.getMaxWeight() + ">");
    }
